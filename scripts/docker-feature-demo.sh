...
# Run a command for live verification or artifact generation without a README screenshot.
run() {
  local slug="$1"
  shift
  ...
  } | redact >"${WORKDIR}/logs/${slug}.txt"
  echo "ran ${slug} exit=${rc}"
  return 0
}
