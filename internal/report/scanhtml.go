...
func scanRiskScore(summary scanHTMLSummary) (int, string) {
	score := 100 - summary.Critical*22 - summary.High*12 - summary.Medium*6 - summary.Low*2 - summary.Exploitable*8
	if score < 0 {
		score = 0
	}
	switch {
...
<header class="masthead">
  ...
  <div class="score"><div class="score-value">{{.Score}}</div><div class="score-label">{{.Posture}}</div><small>Risk posture / 100</small></div>
</header>
...
<section class="section" id="executive">
  ...
  <div class="grid grid-4" style="margin-top:14px">
    <div class="card"><div class="metric-label">Findings</div><div class="metric-value">{{.Summary.Findings}}</div><div class="metric-detail">{{.Summary.Exploitable}} exploitable-class</div></div>
    <div class="card"><div class="metric-label">Targets with findings</div><div class="metric-value">{{.Summary.Targets}}</div><div class="metric-detail">{{.Coverage.Succeeded}} probed successfully</div></div>
    <div class="card"><div class="metric-label">CVE identifiers</div><div class="metric-value">{{.Summary.CVEs}}</div><div class="metric-detail">Potential signature matches</div></div>
    <div class="card"><div class="metric-label">Probe failures</div><div class="metric-value">{{.Coverage.Failed}}</div><div class="metric-detail">{{.Coverage.Submitted}} submitted</div></div>
  </div>
  <div class="status-row" style="margin-top:14px">
    <div class="status-cell"><span>Risk posture</span><strong>{{.Posture}}</strong><small class="muted">Score {{.Score}} / 100 from severity and exploitable-class findings.</small></div>
    <div class="status-cell"><span>Critical</span><strong>{{.Summary.Critical}}</strong></div>
    <div class="status-cell"><span>High</span><strong>{{.Summary.High}}</strong></div>
    <div class="status-cell"><span>Medium</span><strong>{{.Summary.Medium}}</strong></div>
    <div class="status-cell"><span>Low</span><strong>{{.Summary.Low}}</strong></div>
    <div class="status-cell"><span>Informational</span><strong>{{.Summary.Info}}</strong></div>
  </div>
</section>
...
<section class="section" id="risks">
  ...
  {{if .TopRisks}}<div class="grid grid-3">{{range .TopRisks}}<article class="card risk {{.SeverityClass}}">{{if .Exploitable}}<span class="badge EXPLOITABLE">EXPLOITABLE</span> {{end}}<span class="badge {{.SeverityClass}}">{{.SeverityClass}}</span><h3>{{.Title}}</h3><div class="resource">{{.Target}}{{if .CVE}} · {{.CVE}}{{end}}</div><p>{{.Impact}}</p></article>{{end}}</div>{{else}}<div class="card empty success">No scored security findings were emitted for the endpoints that were probed.</div>{{end}}
</section>
...
<section class="section" id="findings">
  <div class="section-head"><h2>Detailed Findings</h2><span class="section-note">Cause, impact, cost if ignored, remediation, and residual risk</span></div>
  <div class="grid">{{range .Findings}}<article class="card finding {{.SeverityClass}}">
    ...
    </div>{{if .Evidence}}<dl class="evidence">{{range .Evidence}}<dt>{{.Code}}</dt><dd>{{if .Summary}}{{.Summary}}{{else}}—{{end}}</dd>{{end}}</dl>{{end}}
...
<section class="section" id="coverage"><div class="section-head"><h2>Assessment Coverage and Scanner Telemetry</h2><span class="section-note">Absence of a finding is meaningful only for successfully probed Elasticsearch endpoints</span></div>
  <div class="grid grid-4">
    <div class="card"><div class="metric-label">Probes submitted</div><div class="metric-value">{{.Coverage.Submitted}}</div><div class="metric-detail">{{.Coverage.Succeeded}} succeeded · {{.Coverage.Failed}} failed</div></div>
    <div class="card"><div class="metric-label">Findings</div><div class="metric-value">{{.Summary.Findings}}</div><div class="metric-detail">{{.Summary.Exploitable}} exploitable-class</div></div>
    <div class="card"><div class="metric-label">Duration</div><div class="metric-value">{{duration .Coverage.Duration}}</div><div class="metric-detail">GET-only assessment</div></div>
    <div class="card"><div class="metric-label">Scanner</div><div class="metric-value">{{if .ScannerVersion}}{{.ScannerVersion}}{{else}}garga{{end}}</div><div class="metric-detail">Finding schema 0.1</div></div>
  </div>
</section>
