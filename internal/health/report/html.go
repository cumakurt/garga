...
func writeHTML(output io.Writer, report healthmodel.Report) error {
	...
	document := healthHTMLDocument{Report: report, LogoBase64: garga.LogoPNGBase64(), Generated: time.Now().UTC()}
...
<header class="masthead">
  ...
  <div class="score"><div class="score-value">{{.Report.Summary.HealthScore}}</div><div class="score-label">{{.Report.Summary.OverallHealth}}</div><small>Overall score / 100</small></div>
</header>
...
<section class="section" id="executive">
  ...
  <div class="grid grid-4">
    <div class="card"><div class="metric-label">Cluster</div><div class="metric-value">{{if .Report.Cluster.Name}}{{.Report.Cluster.Name}}{{else}}Unknown{{end}}</div><div class="metric-detail mono">{{.Report.Cluster.UUID}}</div></div>
    <div class="card"><div class="metric-label">Elasticsearch</div><div class="metric-value">{{.Report.Cluster.Version.Number}}</div><div class="metric-detail">{{.Report.Cluster.Version.BuildFlavor}} / {{.Report.Cluster.Version.BuildType}}</div></div>
    <div class="card"><div class="metric-label">Topology</div><div class="metric-value">{{.Report.Summary.Nodes}} nodes</div><div class="metric-detail">{{.Report.Cluster.DataNodes}} data nodes</div></div>
    <div class="card"><div class="metric-label">Data footprint</div><div class="metric-value">{{bytes .Report.Summary.TotalDataBytes}}</div><div class="metric-detail">{{.Report.Summary.Indices}} indices · {{.Report.Summary.Shards}} shards · {{.Report.Cluster.Documents}} documents</div></div>
  </div>
  <div class="status-row" style="margin-top:14px">
    <div class="status-cell"><span>Cluster API status</span><strong>{{if .Report.Metrics.ClusterHealth.Status}}{{.Report.Metrics.ClusterHealth.Status}}{{else}}unknown{{end}}</strong><small class="muted">Overall health also includes every operational domain.</small></div>
    <div class="status-cell"><span>Critical</span><strong>{{severityCount .Report.Summary.SeverityCounts "CRITICAL"}}</strong></div>
    <div class="status-cell"><span>High</span><strong>{{severityCount .Report.Summary.SeverityCounts "HIGH"}}</strong></div>
    <div class="status-cell"><span>Medium</span><strong>{{severityCount .Report.Summary.SeverityCounts "MEDIUM"}}</strong></div>
    <div class="status-cell"><span>Low</span><strong>{{severityCount .Report.Summary.SeverityCounts "LOW"}}</strong></div>
    <div class="status-cell"><span>Informational</span><strong>{{severityCount .Report.Summary.SeverityCounts "INFO"}}</strong></div>
  </div>
</section>
...
<footer class="footer">Generated by garga · Read-only Elasticsearch Health Assessment Engine · No external scripts or network resources</footer>
</main>
</body>
</html>
`
